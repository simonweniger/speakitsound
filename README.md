...
### A React Hook for easy Text to Speach, Speach to Speach and Sound Effects
...
```bash
yarn add speakitsound
```

Or, use NPM:

```bash
npm install speakitsound
```
...
You can take a look here **[view the storybook](https://speakitsound.vercel.com/)**, which includes lots of quick examples.

---

## Examples

### Text to Speach on click

```js
import useVoice from 'speakitsound';
...
const HelloWorldButton = () => {
  ...
  return <button onClick={play}>Say Hello</button>;
};
```
...
```js
import useSound from 'speakitsound';
...
`speakitsound` requires a path to an audio file, and it isn't obvious how to provide one in a React application.
...
`speakitsound` takes advantage of this: because we know that sounds won't be needed immediately on-load, we can lazy-load a third-party dependency.

`speakitsound` will add about 1kb gzip to your bundle, and will asynchronously fetch an additional package after load, which clocks in around 9kb gzip.
...
const [play] = speakitsound('/path/to/sound', { playbackRate });
```

`playbackRate` doesn't just serve as an _initial_ value for the sound effect. If `playbackRate` changes, the sound will immediately begin playing at a new rate. This is true for all options passed to the `speakitsound` hook.

---

## API Documentation

The `speakitsound` hook takes two arguments:
...
When calling `speakitsound`, you can pass it a variety of options:
...
- `sprite` allows you to use a single `speakitsound` hook for multiple sound effects. See [“Sprites”](https://github.com/joshwcomeau/use-sound#sprites) below.
...
```js
const [play] = speakitsound('/meow.mp3');
...
```js
const [play, exposedData] = speakitsound('/meow.mp3');
//                ^ What we're talking about
```
...
```js
const [play] = speakitsound('/path/to/sprite.mp3', {
  sprite: {
    laser: [0, 300],
    explosion: [1000, 300],
    meow: [2000, 75],
...
Howler is a very powerful library, and we've only exposed a tiny slice of what it can do in `speakitsound`. We expose two escape hatches to give you more control.
...
```js
const [play] = speakitsound('/thing.mp3', {
  onend: () => {
...
```js
const Arcade = () => {
  const [play, { sound }] = speakitsound('/win-theme.mp3');
